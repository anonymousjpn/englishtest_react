...
const Home = () => {
    ...
    return (
        <div>
            ...
            <h2>このサイトについて</h2>
            <p>このサイトでは、英語力を測る指標として日本ではよく利用される、英語検定を活用しています。まずは、下記左の表を参考にしつつ、自分が挑戦したい級のページを開き、英単語問題10問、英熟語問題10問の計20問を解答して下さい。その後、正答率に応じてあなたが挑戦した級のおおまかな合否可能性が表示されます。基本的には、合格可能性が60%～80%を達成できた級が合格を目標にすべき級（＝自分の現在の実力に近い級）になります。もし合格可能性が0%～40%であれば、1つ下のレベルの級の問題も解いてみて下さい。また、合格可能性が90%～100%の場合は、1つ上のレベルの級の問題にも挑戦してみて下さい。英語検定の受験を検討している場合は、英語学習のモチベーションを最大限維持するためには、自分の実力に近い級を特定し、その級の合格に向けて学習することが重要です。</p>
            <p>英語検定以外の資格・検定試験の受験を検討している方は、下記右の表を参考にし、自分の実力に近い級の合格スコアと同じ列の受験予定の試験のスコア（範囲が設定されている場合は範囲内の最高スコア）を目標にして下さい（例：自分の現在の実力に近い級が英検2級であり、IELTSを受験予定の場合はIELTS5.0を目標スコアとして設定する。）。</p>
            <div className="flexdisplay3">
                <figure id="lefttable">
                    <div id="threecontent">
                        <figcaption className="figtext" id="topfig">各級の受験対象者の目安</figcaption>
                        <table border="1" id="tablesize">
                            <tbody>
                                <tr>
                                    <th id="grade">級</th>
                ...
                <figure id="rightimage">
                    <figcaption className="figtext">各試験・検定試験とCEFRとの対照表</figcaption>
